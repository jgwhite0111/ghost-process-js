...
# ---------- 9. alf_tv — cheesy upbeat 80s sitcom theme (just for fun) ----
SCENES["alf_tv"] = {
    "name": "alf_tv",
    "bars": 24,                              # ~96s at 120 BPM (sitcom length)
    "bpm": 120,
    "lead": {"prog": 61, "vol": 100, "pan": 64, "reverb": 30, "mod_init": 0},  # Brass Section
    "bass": {"prog": 34, "vol": 100, "reverb": 20},   # Picked Bass (bouncy)
    "pad":  {"prog": 48, "vol": 80, "pan": 64, "reverb": 50},   # String Ensemble
    "drums": {"vol": 100, "reverb": 20},     # full kit
    "key_intervals": MAJOR,
    "root": 0,                               # C major (cheerful!)
    "lead_mod_ramp": (0, 0),                 # brass stays clean
    "pad_chords": [
        (0, [N(0,4), N(4,4), N(7,4), N(11,4)]),      # C
        (4, [N(7,4), N(11,4), N(2,5), N(5,5)]),      # G
        (8, [N(9,3), N(0,4), N(4,4), N(7,4)]),       # Am
        (12, [N(5,4), N(9,4), N(0,5), N(4,5)]),      # F
    ],
    "lead_pattern": [],
    "bass_pattern": [],
    "drum_pattern": [],
...
def _build_alf_tv_patterns():
    """80s sitcom: bouncey bass, brass stabs, full kit. The catchiest
    4-chord progression of all time."""
    bar = PPQ * BEATS_PER_BAR
    eighth = PPQ // 2
    cfg = SCENES["alf_tv"]
    # Bass: bouncy root-fifth pattern (oom-pah style)
    bass_ev = []
    chord_roots = [0, 7, 9, 5]
    for b in range(cfg["bars"]):
        ...
        for beat in range(4):
            t = b * bar + beat * PPQ
            # oom (downbeat) - pah (offbeat)
            if beat % 2 == 0:
                bass_ev.append((t, [(root, eighth, 0), (fifth, eighth, 0)]))
            else:
                bass_ev.append((t, [(fifth, eighth, 0), (root, eighth, 0)]))
    cfg["bass_pattern"] = bass_ev
    # Lead: brass riff in classic sitcom style — repeated hook
    lead_ev = []
    # 4-bar brass riff (call), 4-bar variation (response)
    riff_call = [N(4,4), N(7,4), N(4,4), N(0,4), N(7,4), N(4,4), N(2,4), N(4,4)]
    riff_resp = [N(7,4), N(4,4), N(7,4), N(11,4), N(7,4), N(4,4), N(2,4), N(0,4)]
    for b in range(0, cfg["bars"], 4):
        riff = riff_call if (b // 4) % 2 == 0 else riff_resp
        for i, n in enumerate(riff):
            t = b * bar + i * eighth
            lead_ev.append((t, [(n, eighth, 0)]))
    cfg["lead_pattern"] = lead_ev
    # Drums: classic 80s sitcom — kick, snare, hat 8ths, open hat on 4
    drum_ev = []
    for b in range(cfg["bars"]):
        for beat in range(4):
            t = b * bar + beat * PPQ
            # Kick on 1 & 3 (with extra on the & of 2 for that 80s feel)
            if beat == 0:
                drum_ev.append((t, KICK, 100))
            if beat == 2:
                drum_ev.append((t, KICK, 95))
                drum_ev.append((t + PPQ // 2, KICK, 85))
            # Snare on 2 & 4
            if beat in (1, 3):
                drum_ev.append((t, SNARE, 100))
            # Hat 8ths
            for e in range(2):
                hat = HHAT_OPEN if (beat == 3 and e == 1) else HHAT
                hat_v = 70 if e == 0 else 60
                drum_ev.append((t + e * eighth, hat, hat_v))
    cfg["drum_pattern"] = drum_ev
_build_alf_tv_patterns()
